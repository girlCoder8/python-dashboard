...
requirement_iii = master_df[['Sales', 'Discount', 'Cost', 'Profit', 'ProductCategory']].groupby(
    'ProductCategory').sum().reset_index()

viz_scatter_iii = px.scatter(requirement_iii, x="Sales", y="Profit", size="Profit", color="ProductCategory",
                             hover_name="Profit", log_x=True, log_y=True, size_max=50, template="seaborn",
                             title='SALES AGAINST PROFIT BY PRODUCT CATEGORY')
...
requirement_ii['Field'] = ' City' + '<br>' + requirement_ii['City'] + '<br>Sales ' + '$' + (
    requirement_ii['Sales'].round(2).astype(str))
...
map_data = px.scatter_mapbox(requirement_ii, lat="Latitude", lon="Longitude", color="Sales", size="Sales",
                             ...
                             hover_data={'City': False, 'Latitude': False, 'Longitude': False, 'Sales': False,
                                         'Field': True}, title='SALES BY CITY', template="seaborn")
...
# --------------------------------------------------------------------------------
# Dash Table
data_table = master_df[
    ['Year', 'Quarter', 'Country', 'City', 'CustomerName', 'ProfitType', 'ProductCategory', 'Sales', 'Profit', 'Cost']]
